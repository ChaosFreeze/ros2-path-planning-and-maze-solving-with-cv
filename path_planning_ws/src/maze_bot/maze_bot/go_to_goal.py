...
class Mazebot_GTG(Node):
    ...
    def goal_movement(self):
        ...
        self.distance_to_goal = math.sqrt(math.pow((self.goal_pose.x - self.robot_pose.x), 2) + math.pow((self.goal_pose.y - self.robot_pose.y), 2))
        self.angle_to_goal = math.atan2(self.goal_pose.y - self.robot_pose.y, self.goal_pose.x - self.robot_pose.x) + self.angle_offset
        self.angle_to_goal -= self.robot_pose.z
